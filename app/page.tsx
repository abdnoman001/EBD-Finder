        ...
        {currentBooks.map((book, index) => (
          <div key={index} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition border border-gray-100 flex flex-col">
            <div className="relative h-64 w-full bg-gray-200">
              ...
              />
              <span className={`absolute top-2 right-2 px-2 py-1 text-xs font-bold text-white rounded ${book.source === 'Rokomari' ? 'bg-green-600' :
                  book.source === 'Wafilife' ? 'bg-cyan-600' :
                    book.source === 'Batighor' ? 'bg-red-600' : 'bg-orange-500'
                }`}>
                {book.source}
              </span>
            </div>
